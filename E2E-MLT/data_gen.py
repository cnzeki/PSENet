...
def draw_box_points(img, points, color=(0, 255, 0), thickness=1):
    try:
        cv2.line(img, (int(points[0][0]), int(points[0][1])), (int(points[1][0]), int(points[1][1])), (255, 0, 0),
                 thickness)
        cv2.circle(img, (int(points[1][0]), int(points[1][1])), 10, (0, 255, 0), -1)
        cv2.line(img, (int(points[2][0]), int(points[2][1])), (int(points[1][0]), int(points[1][1])), (0, 0, 255),
                 thickness)
...
def random_rotation(img, word_gto):
    ...
    word_gto[:, 0, 0] = ((wor[:, 0, 0] - center[0]) * math.cos(angle_rad)) - (
                (wor[:, 0, 1] - center[1]) * math.sin(angle_rad)) + center[0]
    word_gto[:, 0, 1] = ((wor[:, 0, 0] - center[0]) * math.sin(angle_rad)) + (
                (wor[:, 0, 1] - center[1]) * math.cos(angle_rad)) + center[1]

    word_gto[:, 1, 0] = ((wor[:, 1, 0] - center[0]) * math.cos(angle_rad)) - (
                (wor[:, 1, 1] - center[1]) * math.sin(angle_rad)) + center[0]
    word_gto[:, 1, 1] = ((wor[:, 1, 0] - center[0]) * math.sin(angle_rad)) + (
                (wor[:, 1, 1] - center[1]) * math.cos(angle_rad)) + center[1]

    word_gto[:, 2, 0] = ((wor[:, 2, 0] - center[0]) * math.cos(angle_rad)) - (
                (wor[:, 2, 1] - center[1]) * math.sin(angle_rad)) + center[0]
    word_gto[:, 2, 1] = ((wor[:, 2, 0] - center[0]) * math.sin(angle_rad)) + (
                (wor[:, 2, 1] - center[1]) * math.cos(angle_rad)) + center[1]

    word_gto[:, 3, 0] = ((wor[:, 3, 0] - center[0]) * math.cos(angle_rad)) - (
                (wor[:, 3, 1] - center[1]) * math.sin(angle_rad)) + center[0]
    word_gto[:, 3, 1] = ((wor[:, 3, 0] - center[0]) * math.sin(angle_rad)) + (
                (wor[:, 3, 1] - center[1]) * math.cos(angle_rad)) + center[1]
...
def generate_rbox2(im, im_size, polys, tags, labels, vis=False):
    ...
    for poly_idx, poly_tag in enumerate(zip(polys, tags)):
        ...
        txt = labels[poly_idx]
        pts_orig = poly_tag[0]
        angle = (math.atan2((pts_orig[2][1] - pts_orig[1][1]), pts_orig[2][0] - pts_orig[1][0]) + math.atan2(
            (pts_orig[3][1] - pts_orig[0][1]), pts_orig[3][0] - pts_orig[0][0])) / 2
        ...
        if tag == True or poly_h < 6 or poly_w < 6 or np.sum(pts < 0) != 0 or pts_orig[:, 0].max() > im.shape[
            1] or pts_orig[:, 1].max() > im.shape[1] or (poly_w < poly_h and len(txt) > 3):
...
def generator(input_size=512, batch_size=4, train_list='/home/klara/klara/home/DeepSemanticText/resources/ims2.txt',
              vis=False, in_train=True, geo_type=0):
            ...
            try:
                ...
                if in_train:
                    if random.uniform(0, 100) < 80:
                        im_name = image_list[
                            int(random.uniform(0, min(19000, image_list.shape[0] - 1)))]  # use with synthetic data
                ...
                if os.path.exists(txt_fn_gt) and (
                        txt_fn_gt.find('/done/') != -1 or txt_fn_gt.find('/icdar-2015-Ch4/') != -1):
                    text_polys, text_tags, labels_txt = load_gt_annoataion(txt_fn_gt,
                                                                           txt_fn_gt.find('/icdar-2015-Ch4/') != -1)
                elif os.path.exists(txt_fn) and (
                        txt_fn.find('/Latin/') != -1 or txt_fn.find('/Arabic/') != -1 or txt_fn.find(
                        '/Chinese/') != -1 or txt_fn.find('/Japanese/') != -1 or txt_fn.find('/Bangla/') != -1):
                    try:
                ...
                im = np.array(pim)
                if geo_type == 0:
                    score_map, geo_map, training_mask, gt_idx, gt_out, labels_out = generate_rbox(im, (new_h, new_w),
                                                                                                  text_polys, text_tags,
                                                                                                  labels_txt, vis=vis)
                else:
                    score_map, geo_map, training_mask, gt_idx, gt_out, labels_out = generate_rbox2(im, (new_h, new_w),
                                                                                                   text_polys,
                                                                                                   text_tags,
                                                                                                   labels_txt, vis=vis)
