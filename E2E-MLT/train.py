...
def process_boxes(images, im_data, iou_pred, roi_pred, angle_pred, score_maps, gt_idxs, gtso, lbso, features, net,
                  ctc_loss, opts, debug=False):
    ctc_loss_count = 0
    ...
    for bid in range(iou_pred.size(0)):
        ...
        for i in range(0, xy_text.shape[0]):
            ...
            angle_gt = (math.atan2((gt[2][1] - gt[1][1]), gt[2][0] - gt[1][0]) + math.atan2((gt[3][1] - gt[0][1]),
                                                                                            gt[3][0] - gt[0][0])) / 2
            ...
            pos_g2 = np.array(
                [(posp[1] + offset[1] * math.sin(angle)) * 4, (posp[0] + offset[1] * math.cos(angle)) * 4])

            pos_r = np.array([(posp[1] - offset[2] * math.cos(angle)) * 4, (posp[0] - offset[2] * math.sin(angle)) * 4])
            pos_r2 = np.array(
                [(posp[1] + offset[3] * math.cos(angle)) * 4, (posp[0] + offset[3] * math.sin(angle)) * 4])
            ...
            probs_sizes = autograd.Variable(
                torch.IntTensor([(labels_pred.permute(2, 0, 1).size()[0])] * (labels_pred.permute(2, 0, 1).size()[1])))
...
def main(opts):
    ...
    for step in range(step_start, opts.max_iters):
        ...
        if step % disp_interval == 0:
            ...
            try:
                print(
                    'epoch %d[%d], loss: %.3f, bbox_loss: %.3f, seg_loss: %.3f, ang_loss: %.3f, ctc_loss: %.3f, rec: %.5f lv2 %.3f' % (
                        step / batch_per_epoch, step, train_loss, bbox_loss, seg_loss, angle_loss, ctc_loss_val,
                        good_all / max(1, gt_all), ctc_loss_val2))
            except:
                import sys, traceback
                traceback.print_exc(file=sys.stdout)
                pass
