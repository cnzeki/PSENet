...
def process_splits(trans, word_splits, conf, splits, start, ctc_f, rot_mat, angle, box_points, w, h, draw, is_dict,
                   debug=False):
    '''
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        for img_name in sorted(images):
            num = int(
                os.path.basename(img_name).replace('ts_', "").replace("img_", "").replace('.jpg', "").replace('.JPG',
                                                                                                              "").replace(
                    '.png', ""))
        ...
        for tp in sorted(nums, key=lambda images: images[0]):
            ...
            im_resized, (ratio_h, ratio_w) = resize_image(img, max_size=1848 * 1024,
            ...
            for bid, box in enumerate(detections):
                ...
                if len(det_text) > 0:
                    ...
                    splits_raw = process_splits(det_text, word_splits, conf_raw, dec_s, conf2, ctc_f, rot_mat, angle,
                                                boxr, w, h, im_resized,
                    ...
                    for spl in splits_raw:
                        ...
                        if len(spl[1][0]) >= eval_text_length:
                            ...
                            res_file.write(
                                '{},{},{},{},{},{},{},{},1.0,{}\r\n'.format(int(boxw[2]), int(boxw[3]), int(boxw[4]),
                                                                            int(boxw[5]), int(boxw[6]), int(boxw[7]),
                                                                            int(boxw[0]), int(boxw[1]), spl[1][0]))
