...
def test(args):
    ...
    for idx, (org_img, img) in enumerate(test_loader):
        ...
        write_result_as_txt(image_name, data_loader.img_paths, bboxes,'output_kuzu/BoundingBoxCords/')
        
        text_box = cv2.resize(text_box, (text.shape[1], text.shape[0]))
        debug(idx, data_loader.img_paths, [[text_box]], 'output_kuzu/Detections/')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--min_score', nargs='?', type=float, default=0.93,
                        help='min score')
    parser.add_argument('--testdir',nargs='?',type=str,default=None,
